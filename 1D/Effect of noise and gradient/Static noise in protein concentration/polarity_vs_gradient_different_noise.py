...
def simulate(t_span,rho,epsilon,noise,alpha, beta):
    ...
    aT = rho*(1+noise_array[0,:,:]) + epsilon*np.linspace(-0.5,0.5,nx)*np.ones((ny,nx))
...
if __name__ == "__main__":
    ...
    for noise in [0,1e-3,5e-3]:
        ...
        epsilon_array = np.sort(np.around(np.concatenate((np.linspace(-0.1,0.1,25),np.linspace(-0.01,0.01,15))),5));
        pa_array=np.empty((len(epsilon_array),no_of_instances));
        pb_array=np.empty((len(epsilon_array),no_of_instances));
    
        for ri, epsilon in tqdm(enumerate(epsilon_array)):
            
            for instance in range(0,no_of_instances,1):
                
                aL_t, bL_t, aR_t, bR_t = simulate(t_span,rho,epsilon,noise,alpha, beta)
                pa_array[ri,instance],pb_array[ri,instance]=plotting_gradient.calculate_pa_pb(aL_t,bL_t,aR_t,bR_t)
        
        data_file=main_folder+"noise="+str(noise)+"_rho="+str(rho)
        np.savez(data_file,noise_amp_array=epsilon_array,pa_array=pa_array,pb_array=pb_array)      
        
        ## Pa vs rho
        axs[0].plot(epsilon_array,np.mean(pa_array,axis=1),'.-',label=str(noise));
        ## fill the standard deviation:
        axs[0].fill_between(epsilon_array,np.mean(pa_array,1)-np.std(pa_array,1),np.mean(pa_array,1)+np.std(pa_array,1),alpha=0.5)
        axs[0].set_ylabel("Polarity of Ft "+r"$(p_{f})$")
        axs[0].set_xlabel("Gradient of Ft "+r"$(\epsilon)$")
        axs[0].legend(title=r"$S$")
        #axs[1].set_aspect("equal")
    
        ## Pb vs rho
        axs[1].plot(epsilon_array,np.mean(pb_array,axis=1),'.-',label=str(noise));
        ## fill the standard deviation:
        axs[1].fill_between(epsilon_array,np.mean(pb_array,1)-np.std(pb_array,1),np.mean(pb_array,1)+np.std(pb_array,1),alpha=0.5)
        axs[1].set_ylabel("Polarity of Ds "+r"$(p_{d})$")
        axs[1].set_xlabel("Gradient of Ft "+r"$(\epsilon)$")
        axs[1].legend(title=r"$S$")
    ...
    f.savefig(main_folder+"polarity_vs_gradient_for_different_noise_above_critical_point"+".png",dpi=800)    
    plt.close()
