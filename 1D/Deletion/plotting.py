...
def plots_at_rho(aL,bL,aR,bR,rho,folder):
    ...
    pa = aR - aL; pb = bR - bL; a0 = aR + aL;
    average_p=pa+pb;delta_p=pa-pb;
    
    delta_p_without_deletion=np.mean(delta_p[-1,0,-100:-50])
    delta_p=delta_p-delta_p_without_deletion
    ## Plotting:
    ...
    ## delta p
    axs[0,1].plot(abs(delta_p[-1,0,int(nx/2):int(nx/2)+deletion_part]));
